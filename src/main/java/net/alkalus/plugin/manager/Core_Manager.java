...
public class Core_Manager {
     ...
     */
    public static void registerPlugin(final IPlugin plugin) {
        AcLog.INFO("[Plugin] " + "Registered " + plugin.getPluginName() + ".");
        mPlugins.put(plugin);
    }

    /**
     * Dynamically loads all class objects within the "gtPlusPlus.plugin"
     * package.
     */
    public static void veryEarlyInit() {
        if (ReflectionUtils
                .dynamicallyLoadClassesInPackage("gtPlusPlus.plugin")) {
    ...
    public static boolean preInit() {
        try {
            for (final IPlugin h : mPlugins) {
                if (h.preInit()) {
                    AcLog.INFO("[Plugin] Completed Pre-Init Phase for "
                            + h.getPluginName() + ".");
                } else {
                    AcLog.INFO("[Plugin] Failed during Pre-Init Phase for "
                            + h.getPluginName() + ".");
                }
            }
            return true;
    ...
    public static boolean init() {
        try {
            for (final IPlugin h : mPlugins) {
                if (h.init()) {
                    AcLog.INFO("[Plugin] Completed Init Phase for "
                            + h.getPluginName() + ".");
                } else {
                    AcLog.INFO("[Plugin] Failed during Init Phase for "
                            + h.getPluginName() + ".");
                }
            }
            return true;
    ...
    public static boolean postInit() {
        try {
            for (final IPlugin h : mPlugins) {
                if (h.postInit()) {
                    AcLog.INFO("[Plugin] Completed Post-Init Phase for "
                            + h.getPluginName() + ".");
                } else {
                    AcLog.INFO("[Plugin] Failed during Post-Init Phase for "
                            + h.getPluginName() + ".");
                }
            }
            return true;
