package net.alkalus.core.util.math;
...
public class MathUtils {
    ...
    /**
     * Returns a psuedo-random number between min and max, inclusive. The
     * difference between min and max can be at most Double.MAX_VALUE - 1.
     *
     * @param min Minimim value
     * @param max Maximim value. Must be greater than min.
    ...
    /**
     * Returns a psuedo-random number between min and max, inclusive. The
     * difference between min and max can be at most Float.MAX_VALUE - 1.
     *
     * @param min Minimim value
     * @param max Maximim value. Must be greater than min.
     ...
     * @return double between min and max, inclusive.
     */
    public static double findPercentage(final double current,
            final double max) {
        return Math.round(((current / max) * 100) * 100.00) / 100.00;
    }

    public static int findPercentageOfInt(final long input, final float percentage) {
    ...
    // Smooth Rounding Function
    /**
     * Returns a integer. The returned number is d rounded to the nearest flat
     * integer. Supports Doubles as input.
     *
     * @param current Current value.
    ...
    public static int roundToClosestMultiple(final double number,
            final int multiple) {
        int result = multiple;
    ...
    // Smooth Rounding Function
    /**
     * Returns a long. The returned number is d rounded to the nearest flat
     * long. Supports Doubles as input.
     *
     * @param current Current value.
    ...
    public static int generateSingularRandomHexValue() {
        ...
        AcLog.WARNING("Operating with " + temp);
        temp = Utils.appenedHexNotationToString(String.valueOf(temp));
        AcLog.WARNING("Made " + temp + " - Hopefully it's not a mess.");
        AcLog.WARNING("It will decode into " + Integer.decode(temp) + ".");
        return Integer.decode(temp);
    }

    public static long[] simplifyNumbersToSmallestForm(
            final long[] inputArray) {
    ...
    public static byte safeByte(final long number) {
        return number > Byte.MAX_VALUE ? Byte.MAX_VALUE : (byte) number;
    }

    public static short safeShort(final long number) {
        return number > Short.MAX_VALUE ? Short.MAX_VALUE : (short) number;
    }

    public static int safeInt(final long number, final int margin) {
        return number > Integer.MAX_VALUE - margin ? Integer.MAX_VALUE - margin
                : (int) number;
    }

    public static int getRandomFromArray(final int[] mValues) {
        final int[] mLargeChanceArray = new int[(mValues.length - 1) * 1000];
        int mValueSelection;
    ...
    public static byte getByteAverage(final AutoMap<Byte> aDataSet) {
        final byte[] aNewSet = new byte[aDataSet.size()];
        for (int u = 0; u < aDataSet.size(); u++) {
            final byte b = getSafeByte(aDataSet.get(u));
            aNewSet[u] = b;
        }
        return getByteAverage(aNewSet);
    }

    public static short getShortAverage(final AutoMap<Short> aDataSet) {
        final short[] aNewSet = new short[aDataSet.size()];
        for (int u = 0; u < aDataSet.size(); u++) {
            final short b = getSafeShort(aDataSet.get(u));
            aNewSet[u] = b;
        }
        return getShortAverage(aNewSet);
    }

    public static int getIntAverage(final AutoMap<Integer> aDataSet) {
        final int[] aNewSet = new int[aDataSet.size()];
        for (int u = 0; u < aDataSet.size(); u++) {
            final int b = getSafeInt(aDataSet.get(u));
            aNewSet[u] = b;
        }
        return getIntAverage(aNewSet);
    }

    public static float getFloatAverage(final AutoMap<Float> aDataSet) {
        final float[] aNewSet = new float[aDataSet.size()];
        for (int u = 0; u < aDataSet.size(); u++) {
            final float b = getSafeFloat(aDataSet.get(u));
            aNewSet[u] = b;
        }
        return getFloatAverage(aNewSet);
    }

    public static long getLongAverage(final AutoMap<Long> aDataSet) {
        final long[] aNewSet = new long[aDataSet.size()];
        for (int u = 0; u < aDataSet.size(); u++) {
            final long b = getSafeLong(aDataSet.get(u));
            aNewSet[u] = b;
        }
        return getLongAverage(aNewSet);
    }

    public static double getDoubleAverage(final AutoMap<Double> aDataSet) {
        final double[] aNewSet = new double[aDataSet.size()];
        for (int u = 0; u < aDataSet.size(); u++) {
            final double b = getSafeDouble(aDataSet.get(u));
            aNewSet[u] = b;
        }
        return getDoubleAverage(aNewSet);
    }

    public static byte getByteAverage(final byte[] aDataSet) {
        if (aDataSet.length <= 0) {
            return 0;
        }
        final int divisor = aDataSet.length;
        byte total = 0;
        for (final byte i : aDataSet) {
            total += i;
        }
        final byte result = safeByte(total / divisor);
        return result;
    }

    public static short getShortAverage(final short[] aDataSet) {
        if (aDataSet.length <= 0) {
            return 0;
        }
        final int divisor = aDataSet.length;
        AcLog.INFO("Calculating Average Short. Divisor: " + divisor);
        short total = 0;
        for (final short i : aDataSet) {
            AcLog.INFO("Adding " + i);
            total += i;
        }
        final short result = safeShort((total / divisor));
        AcLog.INFO("Average: " + result);
        return result;
    }

    public static int getIntAverage(final int[] aDataSet) {
        if (aDataSet.length <= 0) {
            return 0;
        }
        final int divisor = aDataSet.length;
        int total = 0;
        for (final int i : aDataSet) {
            total += i;
        }
        final int result = getSafeInt(total / divisor);
        return result;
    }

    public static float getFloatAverage(final float[] aDataSet) {
        if (aDataSet.length <= 0) {
            return 0;
        }
        final int divisor = aDataSet.length;
        float total = 0;
        for (final float i : aDataSet) {
            total += i;
        }
        final float result = (total / divisor);
        return result;
    }

    public static long getLongAverage(final long[] aDataSet) {
        if (aDataSet.length <= 0) {
            return 0;
        }
        final int divisor = aDataSet.length;
        long total = 0;
        for (final long i : aDataSet) {
            total += i;
        }
        final long result = (total / divisor);
        return result;
    }

    public static double getDoubleAverage(final double[] aDataSet) {
        if (aDataSet.length <= 0) {
            return 0;
        }
        final int divisor = aDataSet.length;
        double total = 0;
        for (final double i : aDataSet) {
            total += i;
        }
        final double result = (total / divisor);
        return result;
    }
    ...
    public static int howManyPlaces(int aValueForGen) {
        ...
        final String a = String.valueOf(aValueForGen);
        return a.length();
    }
     ...
     */
    public static int makeNegative(final int aPositive) {
        if (aPositive > 0) {
            return -aPositive;
        } else if (aPositive < 0) {
            return +aPositive;
    ...
    public static <V> V safeCast(final Object aNumberType) {
        long a1;
        double a2;
        a1 = Long.parseLong(aNumberType.toString());
        a2 = Double.parseDouble(aNumberType.toString());

        if ((aNumberType.getClass() == byte.class)
                || (aNumberType instanceof Byte)) {
            if (a1 >= Byte.MIN_VALUE && a1 <= Byte.MAX_VALUE) {
                final String s = "" + a1;
                final Byte s1 = Byte.valueOf(s);
                return (V) s1;
            }
        } else if ((aNumberType.getClass() == short.class)
                || (aNumberType instanceof Short)) {
            if (a1 >= Short.MIN_VALUE && a1 <= Short.MAX_VALUE) {
                final String s = "" + a1;
                final Short s1 = Short.valueOf(s);
                return (V) s1;

            }
        } else if ((aNumberType.getClass() == int.class)
                || (aNumberType instanceof Integer)) {
            if (a1 >= Integer.MIN_VALUE && a1 <= Integer.MAX_VALUE) {
                final String s = "" + a1;
                final Integer s1 = Integer.valueOf(s);
                return (V) s1;

            }
        } else if ((aNumberType.getClass() == long.class)
                || (aNumberType instanceof Long)) {
            if (a1 >= Long.MIN_VALUE && a1 <= Long.MAX_VALUE) {
                final String s = "" + a1;
                final Long s1 = Long.valueOf(s);
                return (V) s1;
            }
        } else if ((aNumberType.getClass() == float.class)
                || (aNumberType instanceof Float)) {
            if (a2 >= Float.MIN_VALUE && a2 <= Float.MAX_VALUE) {
                final String s = "" + a1;
                final Float s1 = Float.valueOf(s);
                return (V) s1;

            }
        } else if ((aNumberType.getClass() == double.class)
                || (aNumberType instanceof Double)) {
            if (a2 >= Double.MIN_VALUE && a2 <= Double.MAX_VALUE) {
                final String s = "" + a1;
                final Double s1 = Double.valueOf(s);
                return (V) s1;
    ...
    public static byte getSafeByte(final Byte b) {
        final Byte a = safeCast(b);
        return a.byteValue();
    }

    public static short getSafeShort(final Short b) {
        final Short a = safeCast(b);
        return a.shortValue();
    }

    public static int getSafeInt(final Integer b) {
        final Integer a = safeCast(b);
        return a.intValue();
    }

    public static long getSafeLong(final Long b) {
        final Long a = safeCast(b);
        return a.longValue();
    }

    public static float getSafeFloat(final Float b) {
        final Float a = safeCast(b);
        return a.floatValue();
    }

    public static double getSafeDouble(final Double b) {
        final Double a = safeCast(b);
        return a.doubleValue();
    }

    public static long safeCast_IntToLong(final int o) {
        final long i = o;
        return i;
    }

    public static int safeCast_LongToInt(final long o) {
    ...
    public static short safeCast_IntToShort(final int o) {
    ...
    public static int safeCast_ShortToInt(final short o) {
        final int i = o;
        return i;
    }

    public static byte safeCast_ShortToByte(final short o) {
        if (o > Byte.MAX_VALUE) {
    ...
    public static short safeCast_ByteToshort(final byte o) {
        final short i = o;
        return i;
    }
     ...
     * @return - An Integer which will be between the bounds, or a boundary
     *         value.
     */
    public static int balance(final int aInput, final int aMin, final int aMax) {
