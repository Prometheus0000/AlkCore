...
public class ArrayUtils {
    ...
    public static <V> V[] expandArray(final V[] someArray,
            final V newValueToAdd) {
        V[] series = someArray;
     ...
     * = list.stream().map(V::getTextureSet).collect(Collectors.groupingBy(
     * Function.identity(),
     * Collectors.counting())).entrySet().stream().max(Map.Entry.
     * comparingByValue()).map(Map.Entry::getKey); return r.get(); }
     */

    @SuppressWarnings("unchecked")
    public static <V> V[] removeNulls(final V[] v) {
        final List<V> list = new ArrayList<V>(Arrays.asList(v));
        list.removeAll(Collections.singleton(null));
        return (V[]) list.toArray();
    }

    @SuppressWarnings("unchecked")
    public static <T> Set<T> combineSetData(final Set<T> S, final Set<T> J) {
        final Set<T> mData = new HashSet<T>();
        final T[] array1 = (T[]) S.toArray();
        Collections.addAll(mData, array1);
        final T[] array2 = (T[]) J.toArray();
        Collections.addAll(mData, array2);
        return mData;
    }

    public static <A> AutoMap<A> mergeTwoMaps(final AutoMap<A> a, final AutoMap<A> b) {
        final AutoMap<A> c = new AutoMap<A>();
        for (final A g : a.values()) {
            c.put(g);
        }
        for (final A g : b.values()) {
            c.put(g);
        }
        return c;
    }

    public static <T> T[][] rotateArrayClockwise(final T[][] mat) {
     ...
     * if they are duplicates https://stackoverflow.com/a/18003532 Read more:
     * http://javarevisited.blogspot.com/2012/12/how-to-sort-hashmap-java-by-key
     * -and-value.html#ixzz2akXStsGj
     */
    public static <K extends Comparable, V extends Comparable> Map<K, V> sortMapByValues(
        ...
        Collections.sort(entries, new Comparator<Map.Entry<K, V>>() {
            ...
            public int compare(final Entry<K, V> o1, final Entry<K, V> o2) {
                ...
                // to compare alphabetically case insensitive return this
                // instead
    ...
    /**
     * Clones a Map, does not ensure that the objects within the map are
     * cloneable.
     * 
     * @param      <A> - Map Key Type
     * @param      <B> - Map Data Type
     * @param aMap - The Map to clone, try to avoid using maps containing unique
     *             objects.
     */
    public static <A, B> Map<A, B> cloneMap(final Map<A, B> aMap) {
        final Set<Entry<A, B>> aKeys = aMap.entrySet();
        final LinkedHashMap<A, B> aNewMap = new LinkedHashMap<A, B>();
        for (final Entry<A, B> data : aKeys) {
